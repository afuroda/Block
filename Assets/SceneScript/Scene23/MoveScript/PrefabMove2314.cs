...
public class PrefabMove2314 : MonoBehaviour {
    ...
    void Start () {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (i <= 0) {
                ...
                target = Instantiate (obj, new Vector3 (-1.5f, 0.0f, 5.5f), Quaternion.identity);
    ...
    void Update () {
        if (target.transform.position.z >= 6.5) {
            MoveFlag = true;
        } else if (target.transform.position.z <= 5.5) {
            MoveFlag = false;
        }
