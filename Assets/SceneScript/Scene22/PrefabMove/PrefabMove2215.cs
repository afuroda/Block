...
public class PrefabMove2215 : MonoBehaviour {
    ...
    void Start () {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (i <= 0) {
                ...
                target = Instantiate (obj, new Vector3 (4.5f, 0.0f, 4f), Quaternion.identity);
    ...
    void Update () {
        ...
        if (target.transform.position.x >= 4.5 && target.transform.position.z >= 5) {
            MoveFlag = 1;
            ...
            MoveFlag = 3;
        } else if (target.transform.position.x <= 3.5 && target.transform.position.z >= 5) {
            MoveFlag = 4;
        }
