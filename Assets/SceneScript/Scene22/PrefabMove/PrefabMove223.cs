...
public class PrefabMove223 : MonoBehaviour {
    ...
    void Start () {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (i <= 0) {
                ...
                target = Instantiate (obj, new Vector3 (0.5f, 0.0f, 8f), Quaternion.identity);
    ...
    void Update () {
        ...
        if (target.transform.position.x >= 0.5 && target.transform.position.z >= 9) {
            MoveFlag = 1;

        } else if (target.transform.position.x >= 0.5 && target.transform.position.z <= 8) {
            MoveFlag = 2;
        } else if (target.transform.position.x <= -0.5 && target.transform.position.z <= 8) {
            MoveFlag = 3;
        } else if (target.transform.position.x <= -0.5 && target.transform.position.z >= 9) {
            MoveFlag = 4;
        }
