...
public class PrefabMove227 : MonoBehaviour {
    ...
    void Start () {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (i <= 0) {
                ...
                target = Instantiate (obj, new Vector3 (-1.5f, 0.0f, 6f), Quaternion.identity);
    ...
    void Update () {
        ...
        if (target.transform.position.x >= -1.5 && target.transform.position.z >= 7) {
            MoveFlag = 1;
            ...
            MoveFlag = 3;
        } else if (target.transform.position.x <= -2.5 && target.transform.position.z >= 7) {
            MoveFlag = 4;
        }
