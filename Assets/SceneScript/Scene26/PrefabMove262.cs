...
public class PrefabMove262 : MonoBehaviour {
    ...
    void Start () {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (i <= 0) {
                ...
                GameObject obj = (GameObject) Resources.Load ("Blue2Cube");
                // Cubeプレハブを元に、インスタンスを生成、
                target = Instantiate (obj, new Vector3 (4f, 0.0f, 10.0f), Quaternion.identity);
        ...
        MoveFlag = false;
        target.transform.Translate (-0.02f, 0f, 0f);
    }
    ...
    void Update () {
        if (target.transform.position.x >= 4) {
            MoveFlag = true;
        } else if (target.transform.position.x <= 1) {
            MoveFlag = false;
        }
