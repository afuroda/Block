...
public class PrefabMove2810 : MonoBehaviour {
    ...
    void Start () {
        ...
        for (int i = 0; i < 15; i++) {
            int value = Random.Range (9, 10 + 1);
            if (i <= 0) {

                // CubeプレハブをGameObject型で取得
                GameObject obj = (GameObject) Resources.Load ("BlueCube");
                // Cubeプレハブを元に、インスタンスを生成、
                target = Instantiate (obj, new Vector3 (4.5f, 0.0f, 8.5f), Quaternion.identity);
    ...
    void Update () {
        if (target.transform.position.z >= 8.5) {
            MoveFlag = true;
        } else if (target.transform.position.z <= 7.5) {
            MoveFlag = false;
        }
