...
public class PrefabMove152 : MonoBehaviour {
    ...
    void Start () {
        ...
        for (int i = 0; i < 15; i++) {
            ...
            if (i <= 0) {
                ...
                target = Instantiate (obj, new Vector3 ((i * 1.0f), 0.0f, 4.0f), Quaternion.identity);
        ...
        MoveFlag = false;
        target.transform.Translate (-0.05f, 0f, 0.05f);
    }
    ...
    void Update () {
        if (target.transform.position.z >= 4) {
            MoveFlag = true;
        } else if (target.transform.position.z <= -0.5) {
            MoveFlag = false;
        }
